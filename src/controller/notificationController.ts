...
export const AddNotification = expressAsyncHandler(
  async (req: IRequest, res: Response, next: NextFunction) => {
    ...
    if (Role !== "admin") {
      res.status(401);
      throw new Error("You are not authorized to add notification");
    }
    const notifi = await Notification.create({ notifications: notification });
    if (notifi) {
